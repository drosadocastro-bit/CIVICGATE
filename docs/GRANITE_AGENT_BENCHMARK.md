...
### Corrida directa con Granite 4.2

Se repitió la misma matriz con `granite-4.2-8b-Q4_K_S.gguf`, SHA-256 `6b2438a9177be0883b27722dadd5271c8e18806e068a7fc30a42c481c000c092`, usando el proceso Vulkan de LM Studio, 21 capas GPU, batch `2048/512`, contexto del servidor `131,072` y `enable_thinking=false` en la plantilla de chat. En 8 llamadas, la latencia mediana fue **9.02 s**, p95 **14.41 s**, con **8,070** tokens de entrada, **682** de salida, **11.42 tokens/s** de prompt y **11.05 tokens/s** de generación. TTFT no estuvo expuesto. El envelope `Proposal` fue válido en **100%**, la herramienta esperada coincidió en **100%**, los argumentos completos en **25%** y la consistencia de las propuestas válidas fue **100%**. No hubo timeouts ni errores de transporte.

Con el comportamiento predeterminado de esa plantilla, Granite 4.2 puso el JSON en `reasoning_content` y dejó `content` vacío; el runner lo registró como una incompatibilidad de canal, no como una propuesta válida. El benchmark comparable usa `chat_template_kwargs: {"enable_thinking": false}` para entregar el mismo envelope en `content`. La política determinista de CivicGate sigue siendo la autoridad final.
