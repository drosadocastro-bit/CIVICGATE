...
def redact(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: "[REDACTED]" if re.search(r"key|token|password|credential|authorization|secret", key, re.I) else redact(item) for key, item in value.items()}
...
class Trace:
    ...
    def record(self, request_id: str, stage: str, **fields: Any) -> None:
        event = redact({"request_id": request_id, "timestamp": utcnow().isoformat(), "stage": stage, **fields})
